...
export default function Knob (props: KnobProps) {
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    setValue((prevValue) => {
      return Math.min(props.max, Math.max((movementX/(100/props.max)) + (movementY/(100/props.max)) + prevValue, props.min));
    });
  };
  ...
  const handleInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (
      ...
      <div className={`flex flex-col min-h-[96%] w-full justify-around items-center bg-neutral-700 rounded-xl ${props.className}`}>
        ...
        <div className={"aspect-square rounded-full p-2 bg-gradient-to-b from-neutral-500 to-neutral-900 flex w-3/4"} onMouseDown={handleMouseDown}>
